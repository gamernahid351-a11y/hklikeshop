...
type Pkg = {
  ...
  likes_per_day: number; duration_days: number; price_bdt: number;
};
  ...
  useEffect(() => {
    (async () => {
      const [{ data: p }, { data: s }] = await Promise.all([
        supabase.from("packages").select("id,name,description,likes_per_day,duration_days,price_bdt").eq("is_active", true).eq("type", "like").order("sort_order"),
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {pkgs.map((p) => (
          <Card key={p.id} className="bg-card border-border p-5 shadow-card flex flex-col gap-3">
            ...
            <div className="font-display font-extrabold text-3xl">৳{Number(p.price_bdt).toFixed(2)}</div>
            <div className="text-sm flex items-center gap-2"><span className="w-5 h-5 rounded-full bg-primary/20 text-primary grid place-items-center"><BadgeCheck className="w-3 h-3"/></span>Instant Delivery</div>
            ...
            <Button onClick={() => { setSelected(p); setUid(""); }} className="mt-auto w-full bg-foreground text-background hover:bg-foreground/90 font-bold rounded-xl h-11">Select Pack</Button>
          </Card>
        ))}
        ...
        <DialogContent className="bg-card border-border max-w-md">
          ...
          <div className="space-y-4">
            ...
            <Button onClick={buy} disabled={busy} className="w-full bg-gradient-primary text-primary-foreground font-bold h-12">
              {busy ? <Loader2 className="w-4 h-4 animate-spin"/> : <><ThumbsUp className="w-4 h-4 mr-2"/>Buy for ৳{Number(selected?.price_bdt)}</>}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
